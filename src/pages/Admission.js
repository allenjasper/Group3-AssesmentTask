...
/**
 * Admission component
 * - Shows Requirements & Process
 * - Form with validation (same rules as before)
 * - Generate filled PDF (jsPDF)
 * - Preview generated PDF inline (iframe)
 * - Download generated PDF
 * - Download official (blank) PDF from public/files/admission-form.pdf
 */
...
export default function Admission() {
  ...
  const [lastBlob, setLastBlob] = useState(null); // retain blob for download
  ...
  // Create jsPDF document and return Blob (so we can preview or download)
  const createPdfBlob = () => {
    const doc = new jsPDF({ unit: "pt", format: "letter" });

    // add logo if available (client-side path)
    try {
      // if you want to embed a base64 or remote image, ensure CORS or use public path
      doc.addImage("/pnc-logo.png", "PNG", 40, 30, 60, 60);
    } catch (err) {
      // ignore if image couldn't be added
    }
  ...
  // generate and preview PDF (creates blob and sets embed URL)
  ...
  // direct download of the generated PDF (if preview created) or create on the fly
  ...
  // cleanup previewUrl object URL on unmount
  useEffect(() => {
    return () => {
  ...
  // handle full-form submit: generate and auto-download
